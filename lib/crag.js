...
function cleanupParagraphText (text) {
...
function getClimbs ($) {
  ...
  var currentButtress;
  var position = 0;
  var inClimbTable = false;

  $('table.small tr').each(function (i, row) {
    var $row = $(row);

    if (!inClimbTable && $row.attr('bgcolor') !== '#cccccc' && $row.attr('bgcolor') !== '#dddddd') {
      return;
    }
    ...
    if ($row.attr('bgcolor') === '#cccccc') {
      currentButtress = capitalCase($row.find('td.normal').text());
      return;
    }

    var climb = {
      position: position,
      name: $row.find('td').slice(1,2).text(),
      buttress: null
    };
    ...
    climb.grade = getGradeFromRawGrade($row.find('td[nowrap=nowrap]').text().trim());
      ...
      $b.each(function (i, el) {
        ...
        switch ($el.text() ) {
          ...
          case 'Access notes':
            ...
            var notes = [cleanupParagraphText($parent.html().replace(/<!--[\s\S]*-->/, ''))];
            ...
            $ps.each(function (i, el) {
              ...
              if (getParagraphContent) {
                  notes.push(cleanupParagraphText($el.text()));
              }
            ...
            access_notes = access_notes.join("\n\n");
            break;
          case 'Guidebooks':
