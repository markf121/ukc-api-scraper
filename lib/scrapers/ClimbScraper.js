  ...
  ClimbScraper.scrapeVotes = function ($) {
    ...
    if ($table.length) {
      ...
      gradeVoteOptions.forEach(function (key, i) {
        addVote(key, i, votes.gradeType1);
      });

      if ($table.length === 3) {
        votes.gradeType2 = {};
        gradeVoteOptions.forEach(function (key, i) {
          addVote(key, i + offsetIndex, votes.gradeType2);
        });
        offsetIndex += 9;
      }
      ...
      starsVoteOptions.forEach(function (key, i) {
        var $item = $trs.slice(i + offsetIndex, i + offsetIndex + 1).clone(),
            $font,
            matches;
