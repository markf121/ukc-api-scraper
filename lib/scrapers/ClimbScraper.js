...
module.exports = function (Scraper, ClimbRequester, CragScraper, GradeUtils, logger) {
    ...
    if (fa) {
      ...
      if (matches) {
        if (matches.length === 2) {
          ...
          date = matches[2] + '-' + (months.indexOf(matches[1]) + 1);
        } else if (matches.length === 4) {
          date = matches[3] + '-' + (months.indexOf(matches[2]) + 1) + '-' + matches[1];
        }
      }
    }
  ...
  ClimbScraper.scrapeVotes = function ($) {
    var $table = $('table[style~="float:right"]'),
        grades = null,
        $tr,
        total;
    ...
    if ($table.length) {
      ...
      gradeVoteOptions.forEach(function (key, i) {
        ...
        votes.grades.total = parseInt(matches[2], 10);
        votes.grades[key + 'Count'] = parseInt(matches[1], 10);

        if (key === 'harderGrade' || key === 'sameGrade' || key === 'easierGrade') {
          $font = $item.find('font');
          $font.find('i').remove();
          votes.grades[key + 'Value'] = $font.text().trim();
        }
      });

      starsVoteOptions.forEach(function (key, i) {
        var $item = $trs.slice(i + 9, i + 10).clone(),
            $font,
            matches;
