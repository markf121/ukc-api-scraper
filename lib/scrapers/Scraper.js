...
module.exports = function () {
  ...
  Scraper.scrapeCarouselPhotos = function ($, crag, climb) {
    ...
    var $container = $('#main table table tr:nth-child(7)').first().clone(),
        $img = $container.find('img'),
        $font,
        href,
        parts;

    if ($img.length && $img.attr('src') !== 'http://ukc2.com/core/1px.gif') {
      $font = $container.find('p').find('font').first();
      while (!$font.text().match(/^Photo:/)) {
        $font.remove();
        $font = $container.find('p').find('font').first();
      }
      ...
      if (href) {
        ...
        photo = {
          ...
          copyright: parts.length > 1 ? parts[1] : null,
          thumbnail: {
            width: parseInt($img.attr('width'), 10),
            height: parseInt($img.attr('height'), 10)
          }
