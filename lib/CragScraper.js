...
CragScraper.prototype._success = function (obj) {
  var $ = obj.$;
  ...
  console.info(cragData);

  this._addCrag(cragData, this._promise);
};
...
CragScraper.prototype._addCrag = function (cragData, promise) {
  var crag = new this.models.Crag(cragData);
  crag.save();

  this._promise.fulfill(crag);
};
    ...
    if ($el.text() === 'Access notes') {
      ...
      $ps.each(function (i, el) {
        ...
        if (getParagraphContent) {
            notes.push(Scraper.cleanupParagraphText($el.text()));
        }
