...
module.exports = function (logger) {
  ...
  CragSaver.prototype._addReferenceToCountry = function () {
    ...
    this.models.Country.findOne({name: this.data.country}, '_id', function (err, country) {
      ...
      if (!country) {
        err = new Error('Can\'t find country: ' + this.data.country);
        self.log.error(err);
        promise.reject(err);
        return;
      }
  ...
  CragSaver.prototype._addReferenceToArea = function () {
    ...
    this.models.Area.findOne({
      ...
      if (!area) {
        err = new Error('Can\'t find area: ' + this.data.area);
        self.log.error(err);
        promise.reject(err);
        return;
      }
  ...
  CragSaver.prototype._addReferenceToRockType = function () {
    ...
    this.models.RockType.findOne({
      ...
      if (!rockType) {
        err = new Error('Can\'t find rocktype: ' + this.data.rockType);
        self.log.error(err);
        promise.reject(err);
        return;
      } else {
    ...
    if (this.data.climbs) {
      this.data.climbs.forEach(function (data, i) {
        if (data.climbType) {
          ...
          if (!self.data.difficultyGroupCountsByClimbType[data.climbType - 1]) {
            self.data.difficultyGroupCountsByClimbType[data.climbType - 1] = 0;
          }
        }
        var promise = Vow.promise();
        var id = data._id;
          ...
          self.models.Grade.findOne(
            {
              ...
              if (!grade) {
                ...
                if (grade.difficultyGroup !== null) {
                  ...
                  if (!self.data.difficultyGroupCountsByClimbType[data.climbType - 1]) {
                    self.data.difficultyGroupCountsByClimbType[data.climbType - 1] = [0, 0, 0, 0];
                  } else {
                    self.data.difficultyGroupCountsByClimbType[data.climbType - 1][grade.difficultyGroup] += 1;
                  }
                }
              }
