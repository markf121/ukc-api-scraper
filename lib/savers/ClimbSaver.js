...
module.exports = function (logger) {
  ...
  ClimbSaver.prototype.remove = function () {
    var self = this;

    this._promise = Vow.promise();

    this.log.info('Removing Climb:' + this.data._id);

    this.models.Climb.remove({
      _id: this.data._id
    }, function (err) {
      if (err) {
        self.log.error(err);
  ...
  ClimbSaver.prototype._addReferenceToGrade = function () {
    ...
    if (data.grade && data.climbType) {
      self.models.Grade.findOne({
        ...
        self.log.info('Adding ref to grade: ' + data.grade.grade, ', ' + data.grade.type + ', ' + data.climbType);
        if (err) {
          self.log.error(err);
          promise.reject(err);
          return;
        ...
        if (!grade) {
          err = new Error('Can\'t find grade: ' + data.grade.grade, ', ' + data.grade.type + ', ' + data.climbType);
          self.log.error(err);
          delete data.grade;
        } else {
  ...
  ClimbSaver.prototype._addReferencesToPhotos = function () {
    var promises = [];
    var self = this;
